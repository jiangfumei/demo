...
public class DateUtil {
    ...
    public static boolean isSameDay(Calendar cal1, Calendar cal2) {//检查两个日期对象是否同一天忽略时间
        return false;//true,如果两个日期在同一天；
    }

    public static boolean isSameInstant(Date date1, Date date2) {//检查两个日期对象是否代表同一时刻，此方法比较两个日期精确到毫秒数；
        return false; //true,如果两个日期代表相同的毫秒时刻；
    }

    public static boolean isSameInstant(Calendar cal1, Calendar cal2)//此方法比较两个日期精确到毫秒数；
    {
        return false; //true,如果两个日期代表相同的毫秒时刻；
    }

    public static boolean isSameLocalTime(Calendar cal1, Calendar cal2) {//此方法比较两个对象的字段值，另外两个日历必须为同一类型的日历；
        return false;//返回值：true,如果两个日期代表相同的毫秒时刻；
    }
